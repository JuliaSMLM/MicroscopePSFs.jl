...
# Test 2D PSFs
gaussian2d = GaussianPSF(0.15)  # σ = 150nm
airy2d = AiryPSF(1.4, 0.532)    # NA = 1.4, λ = 532nm

pixels_gaussian = integrate_pixels(gaussian2d, camera, emitter2d)
pixels_airy = integrate_pixels(airy2d, camera, emitter2d)

println("2D PSFs:")
println("  GaussianPSF: Sum = $(sum(pixels_gaussian))")
println("  AiryPSF: Sum = $(sum(pixels_airy))")

# Test 3D PSFs
scalar3d = ScalarPSF(1.4, 0.532, 1.518)  # NA = 1.4, λ = 532nm, n = 1.518
vector3d = VectorPSF(1.4, 0.532, dipole)
...
println("\n3D PSFs at z=0:")
println("  ScalarPSF: Sum = $(sum(pixels_scalar_z0))")
println("  VectorPSF: Sum = $(sum(pixels_vector_z0))")
...
println("\n3D PSFs at z=1 μm:")
println("  ScalarPSF: Sum = $(sum(pixels_scalar_z1))")
println("  VectorPSF: Sum = $(sum(pixels_vector_z1))")
println("  SplinePSF: Sum = $(sum(pixels_spline_z1))")

function test_depth_dependence(psf::VectorPSF, max_depth::Real=5.0)
