...
function crlb_poisson(psf, camera, emitter)
    # Calculate the PSF and its derivatives using Enzyme
    dx_values = Enzyme.jacobian(set_runtime_activity(Enzyme.Reverse),
        x -> integrate_pixels(psf, camera, Emitter3D(x, emitter.y, emitter.z, emitter.photons)), 
        emitter.x)[1]
    dy_values = Enzyme.jacobian(set_runtime_activity(Enzyme.Reverse),
        y -> integrate_pixels(psf, camera, Emitter3D(emitter.x, y, emitter.z, emitter.photons)), 
        emitter.y)[1]
    dz_values = Enzyme.jacobian(set_runtime_activity(Enzyme.Reverse),
        z -> integrate_pixels(psf, camera, Emitter3D(emitter.x, emitter.y, z, emitter.photons)), 
        emitter.z)[1]
...
z_range = -1.0:0.1:1.0  # Z stage positions in microns
...
# Plot with CairoMakie
fig = Figure()
ax = Axis(fig[1, 1], title = "CRLB", xlabel="z (μm)", ylabel="σ (μm)", yscale=Log10Scale())
plot!(ax, z_range, σ_x, label="σ_x", color=:red)
plot!(ax, z_range, σ_y, label="σ_y", color=:blue)
plot!(ax, z_range, σ_z, label="σ_z", color=:green)
display(fig)
save("crlb.png", fig)
